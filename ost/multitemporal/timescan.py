...
def date_as_float(date):
    ...
    return date.year + days_from_jan1.days * size_of_day + \
...
def _zvalue_from_index(arr, ind):
    """private helper function to work around the limitation of np.choose()
    by employing np.take()
    arr has to be a 3D array
    ind has to be a 2D array containing values for z-indicies to take from arr
    See: http://stackoverflow.com/a/32091712/4169585
    This is faster and more memory efficient than using the ogrid based solution with
    fancy indexing.
    """
    ...
    idx = n_c * n_r * ind + np.arange(n_c * n_r).reshape((n_c, n_r))
...
def nan_percentile(arr, q):
    ...
    for i in range(len(qs)):
        ...
        # linear interpolation (like numpy percentile) takes the
        # fractional part of desired position
